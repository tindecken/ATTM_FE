...
export default defineComponent({
  ...
  setup() {
    ...
    function createDB() {
      ...
      const createRegression: Promise<any> =
        createRegressionStore.createRegression(
          defineRegression.value as DefineRegressionInterface
        );
      createRegression
        .then((r) => {
          tab.value = 'createRegression';
          ...
          const Ids: any[] = [];
          selectedTestCasesDetail.value.forEach(
            (c: TestCaseDetailInterface) => {
              Ids.push(c.Id);
            }
          );
          const createRegressionTest: Promise<any> =
            createRegressionStore.createRegressionTests({
          ...
          createRegressionTest
            .then((t) => {
              ...
              const generateRegCodeResult: Promise<any> =
                globalStore.generateRegCode(t.data.data);
