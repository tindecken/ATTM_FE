    ...
    <q-tab-panels v-model="tab" animated keep-alive>
      ...
      >
        <p class="text-subtitle1 text-info">
          Deploy Source Code: {{ client.DeploySourceStatus }}
        </p>
        <q-btn
        ...
        <p class="text-subtitle1 text-info">
          Update release in setting file: {{ client.UpdateReleaseStatus }}
        </p>
        <q-btn
          v-if="
...
export default defineComponent({
  ...
  setup() {
    ...
    const defineRegression = computed(
      () => createRegressionStore.defineRegression
    );
    function retryDeploySource(client: TestClientInterface) {
      const index: number = selectedClients.value.findIndex(
        (c: TestClientInterface) => c.Id === client.Id
      );
    ...
    function retryUpdateReleaseSetting(client: TestClientInterface) {
      ...
      const updateReleaseResult: Promise<any> =
        globalStore.updatereleaseforclient(payload);
      updateReleaseResult
        .then((r: any) => {
    ...
    function getSelectedString() {
      return selectedClients.value.length === 0
        ? ''
        : `${selectedClients.value.length} record${
            selectedClients.value.length > 1 ? 's' : ''
          } selected of ${clients.value.length}`;
    }
    function deployClient() {
      ...
      selectedClients.value.forEach(
        (testClient: TestClientInterface, index: number) => {
          ...
          const copyResult: Promise<any> =
            globalStore.copyregcodetoclient(cloneClient);
          copyResult
            .then((r) => {
