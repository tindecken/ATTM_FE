  ...
  actions: {
    ...
    createTestSuite_updateState(responseTestSuite: TestSuiteInterface) {
      const catIndex = this.Categories.findIndex(
        (cat: CategoryInterface) => cat.Id === responseTestSuite.CategoryId
      );
    ...
    deleteTestSuite_updateState(testSuite: TestSuiteInterface) {
      const catIndex = this.Categories.findIndex(
        (cat: CategoryInterface) => cat.Id === testSuite.CategoryId
      );
      const tempCat = _.cloneDeep(this.Categories[catIndex]);
      // find tsIndex
      const tsIndex = tempCat.children.findIndex(
        (ts: TestSuiteInterface) => ts.Id === testSuite.Id
      );
      // find tgIndex
    ...
    createTestGroup(testGroup: TestGroupInterface) {
      const catIndex = this.Categories.findIndex(
        (cat: CategoryInterface) => cat.Id === testGroup.CategoryId
      );
      const tempCat = _.cloneDeep(this.Categories[catIndex]);
      // find tsIndex
      const tsIndex = tempCat.TestSuiteIds.findIndex(
        (tsId: string) => tsId === testGroup.TestSuiteId
      );
    ...
    deleteTestGroup(testGroup: TestGroupInterface) {
      ...
      // find tsIndex
      const tsIndex = tempCat.children.findIndex(
        (ts: TestSuiteInterface) => ts.Id === testGroup.TestSuiteId
      );
      // find tgIndex
      const tgIndex = tempCat.children[tsIndex].children.findIndex(
        (tg: TestGroupInterface) => tg.Id === testGroup.Id
      );
    ...
    updateTestCase(testCase: TestCaseInterface) {
      const catIndex = this.Categories.findIndex(
        (cat: CategoryInterface) => cat.Id === testCase.CategoryId
      );
      console.log('catIndex', catIndex);
      const tempCat = _.cloneDeep(this.Categories[catIndex]);
      // find tsIndex
      const tsIndex = tempCat.children.findIndex(
        (ts: TestSuiteInterface) => ts.Id === testCase.TestSuiteId
      );
      console.log('tsIndex', tsIndex);
      // find tgIndex
      const tgIndex = tempCat.children[tsIndex].children.findIndex(
        (tg: TestGroupInterface) => tg.Id === testCase.TestGroupId
      );
      console.log('tgIndex', tgIndex);
      const tcIndex = tempCat.children[tsIndex].children[
        tgIndex
      ].children.findIndex((tc: TestCaseInterface) => tc.Id === testCase.Id);
    ...
    createTestCase(testCase: TestCaseInterface) {
      ...
      // find tsIndex
      const tsIndex = tempCat.TestSuiteIds.findIndex(
        (tsId: string) => tsId === testCase.TestSuiteId
      );
      // find tgIndex
      const tgIndex = tempCat.children[tsIndex].TestGroupIds.findIndex(
        (tgId: string) => tgId === testCase.TestGroupId
      );
      ...
      tempCat.children[tsIndex].children[tgIndex].TestCaseIds?.push(
        testCase.Id
      );
      this.Categories[catIndex] = tempCat;
    },
    deleteTestCases(testCases: TestCaseInterface[]) {
      testCases.forEach((tc: TestCaseInterface) => {
        const catIndex = this.Categories.findIndex(
          (cat: CategoryInterface) => cat.Id === tc.CategoryId
        );
        const tempCat = _.cloneDeep(this.Categories[catIndex]);
        // find tsIndex
        const tsIndex = tempCat.children.findIndex(
          (ts: TestSuiteInterface) => ts.Id === tc.TestSuiteId
        );
        // find tgIndex
        const tgIndex = tempCat.children[tsIndex].children.findIndex(
          (tg: TestGroupInterface) => tg.Id === tc.TestGroupId
        );
        const tcIndex = tempCat.children[tsIndex].children[
          tgIndex
        ].children.findIndex((t: TestCaseInterface) => t.Id === tc.Id);
        tempCat.children[tsIndex].children[tgIndex].TestCaseIds?.splice(
          tcIndex,
          1
        );
